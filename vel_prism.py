...
def get_star_spec(scale,profile,u1,u2,planet_y,planet_radius,spots=[],vsini=0,save_star=False,nproc=4,spot_profile=False,elements=50):

  if save_star == False:
    grid, vel_grid, spots = make_star(scale,u1,u2,spots=spots,elements=elements,vsini=vsini)
...
def vel_prism(radiusratio,system_scale,u1,u2,inclination,times,midtransit,period, profile, planet_absorb,atm_radius, east_offset=0,west_offset=0,planet_K=0,star_K=0,vsini=0,plotting=True,get_star_profile=False,save_star=False,result_wvl=False,nproc=4,location='full',absolute=False,spot_profile=False,spots=[],elements=50):
  ...
  the_star = get_star_spec(scale,profile,u1,u2,planet_y,planet_radius,vsini=vsini,spots=spots,save_star=save_star,nproc=nproc,spot_profile=spot_profile,elements=elements)
...
def make_star(scale,u1,u2,spots=[],vsini=0,elements=50):
  ...
  for i in range(0,n-1):          # Begin small filling loop
    ...
    if(len(star) > 0):
      ...
      for j in range(0,len(star)):
        grid[star[j],i]= (1-u1*(1-m[j])-u2*(1-m[j])**2)  # Fills the grid slice i at the x values generated by star with the star and the appropirate LD value.
